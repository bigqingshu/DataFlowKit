...
class StdioWorker:
    ...
    def _dispatch(self, action, payload):
        ...
        if action == "apply_node_config_command":
            node = payload.get("node")
            node_type = payload.get("node_type") or payload.get("type") or payload.get("node_type_id")
            if isinstance(node, dict):
                node_type = node_type or node.get("node_type_id") or node.get("type")
        ...
        if action == "build_data_source_panel_state":
        ...
        if action == "resolve_plugin_parameter_options":
            plugin_id = payload.get("plugin_id") or payload.get("node_type_id")
        ...
        if action == "resolve_plugin_parameter_field_state":
            plugin_id = payload.get("plugin_id") or payload.get("node_type_id")
