...
class PlanWorkflowWindow:
    ...
    def build_writeback_header_section(self, frame):
        ttk.Label(
            frame,
            text=(
    ...
    def build_writeback_source_section(
        self,
        frame,
        config,
        headers,
        ...
        direction_var, direction_combo = self.add_labeled_combo_control(
            frame,
            "写入方向：",
        ...
        write_range_var = self.add_labeled_combo(
            frame,
            "写入范围：",
    ...
    def build_writeback_match_section(self, frame, config, headers, external_columns):
    ...
    def build_writeback_mapping_section(self, frame, config, headers, external_columns, direction):
        if direction == "当前表写入SQLite目标表":
    ...
    def build_writeback_policy_section(self, frame, config):
    ...
    def sync_writeback_match_rules(self, match_section, config):
        config["match_rules"] = []
    ...
    def build_writeback_match_action_buttons(self, match_section, config):
        ...
        def add_match_rule():
            ...
            self.sync_writeback_match_rules(match_section, config)
    ...
    def sync_writeback_mappings(self, mapping_section, config):
        config["field_mappings"] = []
    ...
    def build_writeback_mapping_action_buttons(self, mapping_section, config, writeback_field_state):
        ...
        def add_mapping():
            ...
            self.sync_writeback_mappings(mapping_section, config)
    ...
    def refresh_writeback_policy_widgets(self, direction_var, policy_section):
    ...
    def writeback_external_key(self, direction):
    ...
    def refresh_writeback_fields(
        ...
        cur_external_key = self.writeback_external_key(cur_direction)
        ...
        self.refresh_writeback_policy_widgets(direction_var, policy_section)
    ...
    def build_writeback_config(self, config, headers):
        ...
        frame.pack(fill=tk.BOTH, expand=True, pady=8)
        self.build_writeback_header_section(frame)
        ...
        try:
            if external_table_name:
                external_columns = self.app.get_table_columns(external_table_name)
        except Exception:
            external_columns = []

        source_section = self.build_writeback_source_section(
            frame,
            config,
            headers,
            table_names,
        ...
        match_section = self.build_writeback_match_section(frame, config, headers, external_columns)
        self.build_writeback_match_action_buttons(match_section, config)

        mapping_section = self.build_writeback_mapping_section(frame, config, headers, external_columns, direction)
        left_values = mapping_section["left_values"]
        right_values = mapping_section["right_values"]
        writeback_field_state = {
            "external_columns": list(external_columns),
            "left_values": list(left_values),
            "right_values": list(right_values),
        }
        self.build_writeback_mapping_action_buttons(mapping_section, config, writeback_field_state)

        policy_section = self.build_writeback_policy_section(frame, config)
        ...
        refresh_button.configure(command=lambda: refresh_writeback_fields(True))
