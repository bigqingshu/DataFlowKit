...
class VisualMappingWritePlanTests(unittest.TestCase):
    ...
    def test_describe_config_exposes_visual_mapping_protocol(self):
        ...
        self.assertTrue(described["capabilities"]["action_state"])
        ...
        self.assertEqual(manifest["patch"]["result_schema"], "plugin_config_patch_result.v1")
        self.assertEqual(manifest["patch"]["target_schema"], "plugin_config_patch_target.v1")
        ...
        self.assertEqual(patch_schema["result_schema"]["schema_version"], "plugin_config_patch_result.v1")
        self.assertEqual(patch_schema["result_schema"]["provider"], "PluginService.apply_plugin_config_patch")
        self.assertEqual(patch_schema["target_schema"]["schema_version"], "plugin_config_patch_target.v1")
        patch_target_field_keys = {field["key"] for field in patch_schema["target_schema"]["fields"]}
        self.assertIn("focus_path", patch_target_field_keys)
        self.assertIn("can_focus_item", patch_target_field_keys)
        ...
        self.assertEqual(effect["effect_state"]["schema_version"], "plugin_config_effect_state.v1")
        self.assertEqual(effect["effect_state"]["status"], "warning")
        self.assertIn("提示 1 项", effect["effect_state"]["status_message"])
        self.assertIn("source_file", effect["effect_state"]["expected_output_fields"])
        self.assertEqual(effect["effect_state"]["required_input_tables"][0]["alias"], "文档")
    ...
    def test_plugin_service_applies_visual_mapping_rules_config_patch(self):
        ...
        self.assertEqual(standard_updated["patch"]["patch_context"]["schema_version"], "plugin_config_patch_context.v1")
        self.assertEqual(standard_updated["patch"]["patch_context"]["plugin_id"], visual.PLUGIN_INFO["id"])
        self.assertEqual(standard_updated["patch"]["patch_context"]["operation"], "replace_item")
        ...
        self.assertEqual(service_patch_schema["target_schema"]["schema_version"], "plugin_config_patch_target.v1")
        self.assertEqual(service_patch_schema["result_schema"]["schema_version"], "plugin_config_patch_result.v1")
