...
class TableDataServiceTests(unittest.TestCase):
    ...
    def test_table_editing_helpers_are_ui_free(self):
        ...
        self.assertEqual(state["action_state"]["action_order"][0], "load_clipboard")
        self.assertEqual(state["action_state"]["action_order"][-1], "apply_to_workflow")
        ...
        self.assertEqual(state["action_state"]["actions"]["delete_sqlite"]["disabled_reason"], "当前来源不是 SQLite 表。")
        self.assertTrue(sqlite_actions["action_state"]["is_sqlite_source"])
        self.assertEqual(sqlite_actions["action_state"]["action_order"][6:8], ["save_sqlite", "delete_sqlite"])
        self.assertTrue(sqlite_actions["actions"]["delete_sqlite"]["requires_confirmation"])
        self.assertTrue(sqlite_actions["actions"]["delete_sqlite"]["enabled"])
        self.assertEqual(sqlite_actions["actions"]["delete_sqlite"]["confirmation"]["schema_version"], "data_source_confirmation.v1")
        self.assertEqual(sqlite_actions["actions"]["delete_sqlite"]["confirmation"]["confirmation_field"], "confirmed")
        self.assertIn("{table_name}", sqlite_actions["actions"]["delete_sqlite"]["confirmation"]["message_template"])
        ...
        self.assertEqual(panel_state["action_state"]["action_order"][0], "load_clipboard")
