...
class Qt6UiShellTests(unittest.TestCase):
    ...
    def test_qt_shell_lists_and_configures_plugin_nodes(self):
        ...
        with TemporaryDirectory(dir=os.getcwd()) as temp_dir:
            ...
            self.assertIn("旧窗口多UI策略", controller.node_detail_sections.toPlainText())
            self.assertIn("不可直接打开 .NET、Web、Electron、CLI", controller.node_detail_sections.toPlainText())
            ...
            self.assertIn("旧窗口可直接打开UI：Tk、Qt", controller.legacy_plugin_config_button.toolTip())
            self.assertIn("旧窗口不可直接打开UI：.NET、Web、Electron、CLI", controller.legacy_plugin_config_button.toolTip())
            self.assertIn("旧窗口不支持时：show_migration_prompt", controller.legacy_plugin_config_button.toolTip())
    ...
    def test_plugin_structured_list_buttons_follow_action_state(self):
        ...
        table = widget.findChild(qt.QtWidgets.QTableWidget)
        self.assertIsNotNone(table)
        self.assertEqual(list(widget.plugin_config_buttons.keys())[:2], ["move_item_1", "custom_append"])
    ...
    def test_data_source_manager_pages_sqlite_preview_and_applies_full_table(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertFalse(manager.edit_mode_checkbox.isEnabled())
            self.assertFalse(manager.promote_header_button.isEnabled())
            self.assertIn("分页预览按协议只读", manager.save_button.toolTip())
            self.assertIn("分页预览按协议只读", manager.edit_mode_checkbox.toolTip())
            self.assertIn("分页预览按协议只读", manager.promote_header_button.toolTip())
            self.assertTrue(manager.clear_button.isEnabled())
            self.assertTrue(manager.apply_input_button.isEnabled())
            self.assertTrue(manager.delete_table_button.isEnabled())
