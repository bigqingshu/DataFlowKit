...
- `describe_data_source_service` also returns `client_profiles` and
  ...
  with `release_table_handle` according to `transport_hints.lifecycle.release_when`
  such as source/window changes, full-table loading, or workflow input apply;
  destructive SQLite operations should honor the shared confirmation metadata.
