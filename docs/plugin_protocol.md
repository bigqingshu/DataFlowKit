...
- `protocol_manifest.schema_version: plugin_config_protocol_manifest.v1`：汇总 `describe_plugin_config`、`resolve_plugin_parameter_options`、`preview_plugin_config_effect`、`legacy_config_window` 等接口能力，以及 layout、ui_hints、参数元数据、兼容状态。

Qt 已将 `parameter_field_hints` 合并到参数表单 schema，并可透传 `field_state_index`；其他 UI 应按同样方式渲染标准参数表单，而不是只读取 `PARAMETER_SCHEMA` 的基础字段。
