...
复杂 `structured_list` 视图还应消费服务层生成的 `action_state`，当前 schema 为 `plugin_config_action_state.v1`。它会描述每个按钮的 `visible`、`enabled`、`requires_selection`、`disabled_reason`、`target_offset` 与 `action_id`，例如新增、更新、删除、启用/禁用、上移、下移。UI 不应自行根据行号猜测边界按钮状态，也不应硬编码动作入口；后续 Qt/.NET/Web 都应优先按 `view.action_state.button_order` 排序，再读取 `view.action_state.buttons` 渲染列表工具按钮。
...
- `parameter_metadata.field_state_schema: plugin_parameter_field_state.v1`
- `parameter_metadata.field_state_index`：按字段 key 描述默认可见/启用、条件依赖、候选刷新、warning、empty/invalid 提示，供 Qt/.NET/Web 统一渲染参数字段状态。
- `resolve_plugin_parameter_field_state`：根据当前 config 解析 `visible / enabled / status / needs_options_refresh / dependents`，返回 `plugin_parameter_field_runtime_state.v1`，用于避免各 UI 自行解释条件表达式。
- `protocol_manifest.schema_version: plugin_config_protocol_manifest.v1`：汇总 `describe_plugin_config`、`resolve_plugin_parameter_options`、`resolve_plugin_parameter_field_state`、`preview_plugin_config_effect`、`legacy_config_window` 等接口能力，以及 layout、ui_hints、参数元数据、兼容状态。
- `warning_items[].target.schema_version: plugin_config_warning_target.v1`：由 `PluginService` 统一补齐 `view_id / field / config_path / focus_path / can_focus_view / can_focus_field`，UI 不需要自行从半结构化 warning 猜默认定位字段。
- `apply_plugin_config_patch().patch.patch_context.schema_version: plugin_config_patch_context.v1`：由 `PluginService` 为轻量 patch 和复杂插件 patch 统一补齐操作、视图、编辑器、路径和目标索引摘要；若插件已有自己的 `schema_version`，服务层保留插件协议版本，只补 context。

Qt 已将 `parameter_field_hints` 合并到参数表单 schema，并在插件表单动态刷新时优先通过 `QtHeadlessEngineClient.resolve_plugin_parameter_field_state()` 消费共享字段状态 resolver；其他 UI 应按同样方式渲染标准参数表单，而不是只读取 `PARAMETER_SCHEMA` 的基础字段。

数据源管理也遵循同一思路：`data_source_actions.v1` 会返回 `action_order` 与 `actions`。Qt/.NET/Web 若要动态生成数据源工具栏，应先按 `action_order` 排序，再读取 `actions[action_id]` 的 `enabled`、`disabled_reason`、`requires_confirmation` 和 `confirmation`。
...
- `legacy_window_policy.schema_version: plugin_legacy_window_policy.v1`
- `legacy_window_policy.direct_open_allowed`：旧 GUI 配置窗口只允许 Tk/Qt 兼容入口直接打开；.NET/Web/Electron/CLI 应显示迁移提示并引导到统一配置协议。
- `legacy_window_policy.unsupported_behavior: show_migration_prompt`
...
4. .NET/Web/Electron/CLI 不应直接承接插件自带 GUI 窗口，只消费 `describe_config + parameter_metadata + config_patch` 或显示 `legacy_window_policy` 给出的迁移提示。
5. 插件完成等价协议迁移后，保留旧窗口一段过渡期，再从 UI 主路径移除。
