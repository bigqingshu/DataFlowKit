...
Qt 已将 `parameter_field_hints` 合并到参数表单 schema，并可通过 `QtHeadlessEngineClient.resolve_plugin_parameter_field_state()` 调用共享字段状态 resolver；其他 UI 应按同样方式渲染标准参数表单，而不是只读取 `PARAMETER_SCHEMA` 的基础字段。
